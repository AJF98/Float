  ...
  private async fetchFlightProposals(
    ...
    const { rows } = await query<FlightProposalWithProposerRow>(
      ...
      SELECT
        ...
        fp.airline,
        fp.flight_number,
        COALESCE(fp.departure_airport, fp.origin) AS departure_airport,
        fp.departure_time,
        fp.departure_terminal,
        COALESCE(fp.arrival_airport, fp.destination) AS arrival_airport,
        fp.arrival_time,
        fp.arrival_terminal,
        fp.duration,
        fp.stops,
        ...
        fp.price,
        fp.currency,
        COALESCE(fp.booking_url, (fp.data->>'bookingUrl')::text) AS booking_url,
